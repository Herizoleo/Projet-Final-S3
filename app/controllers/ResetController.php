<?php
/**
 * Contrôleur Reset - Réinitialisation des données
 */
class ResetController {
    ...
    public static function index() {
        ...
        $stats = [
            'besoins' => $db->query("SELECT COUNT(*) as count FROM besoins")->fetch()['count'],
            'dons' => $db->query("SELECT COUNT(*) as count FROM dons")->fetch()['count'],
            'distributions' => $db->query("SELECT COUNT(*) as count FROM distributions")->fetch()['count'],
            'achats' => $db->query("SELECT COUNT(*) as count FROM achats")->fetch()['count'],
            'ventes' => $db->query("SELECT COUNT(*) as count FROM ventes")->fetch()['count'],
            'villes' => $db->query("SELECT COUNT(*) as count FROM villes")->fetch()['count'],
        ];
        
        Flight::render('reset/index', [
            'stats' => $stats,
    ...
    public static function reset() {
        ...
        if (empty($data['confirmation']) || $data['confirmation'] !== 'REINITIALISER') {
            $_SESSION['error'] = "Veuillez taper 'REINITIALISER' pour confirmer.";
            Flight::redirect('/reset');
            return;
        }
        ...
        try {
            ...
            $db->exec("TRUNCATE TABLE types_articles");
            $db->exec("TRUNCATE TABLE categories");
            $db->exec("TRUNCATE TABLE regions");
            $db->exec("DELETE FROM config");
            
            // Réinsérer les données initiales
            self::insertInitialData($db);
            
            // Réactiver les contraintes
            $db->exec("SET FOREIGN_KEY_CHECKS = 1");
            
            $_SESSION['success'] = "Toutes les données ont été réinitialisées avec succès.";
    ...
    /**
     * Réinitialisation complète (y compris données de référence)
     */
    public static function resetComplete() {
        $data = Flight::request()->data;
        
        if (empty($data['confirmation']) || $data['confirmation'] !== 'TOUT SUPPRIMER') {
            $_SESSION['error'] = "Veuillez taper 'TOUT SUPPRIMER' pour confirmer la réinitialisation complète.";
            Flight::redirect('/reset');
            return;
        }
        ...
        try {
            ...
            $_SESSION['success'] = "Réinitialisation complète effectuée. Toutes les données de référence ont été restaurées.";
    ...
    /**
     * Insérer les données initiales depuis le fichier SQL généré
     */
    private static function insertInitialData($db) {
        // Catégories
        $db->exec("INSERT INTO categories (id, nom, description) VALUES
            (1, 'Nature', 'Produits alimentaires et de première nécessité'),
            (2, 'Matériaux', 'Matériaux de construction'),
            (3, 'Argent', 'Dons monétaires')");
        
        // Régions
        $db->exec("INSERT INTO regions (id, nom) VALUES
            (1, 'Atsimo-Atsinanana'),
            (2, 'Atsinanana'),
            (3, 'Diana'),
            (4, 'Menabe'),
        ...
        // Villes
        $db->exec("INSERT INTO villes (id, nom, region_id, population_sinistree) VALUES
            (1, 'Toamasina', 2, 50000),
            (2, 'Mananjary', 5, 30000),
        ...
        // Types d'articles
        $db->exec("INSERT INTO types_articles (id, nom, unite, categorie_id, prix_unitaire) VALUES
            (1, 'Riz', 'kg', 1, 3000),
            (2, 'Eau', 'L', 1, 1000),
        ...
        // Besoins
        $db->exec("INSERT INTO besoins (id, ville_id, type_article_id, quantite_necessaire, quantite_recue, date_enregistrement, statut) VALUES
        ...
        // Dons
        $db->exec("INSERT INTO dons (id, type_article_id, quantite_totale, quantite_disponible, donateur, date_reception, description) VALUES
