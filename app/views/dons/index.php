<?php ob_start(); ?>

<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <a href="/" class="btn btn-outline-secondary btn-sm">
            <i class="bi bi-arrow-left me-1"></i>Retour
        </a>
    </div>
    <a href="/dons/create" class="btn btn-primary">
        <i class="bi bi-plus me-1"></i>Enregistrer un Don
    </a>
</div>

<!-- Filtres de recherche -->
<div class="card mb-4">
    <div class="card-header py-2">
        <a class="text-decoration-none d-flex justify-content-between align-items-center" 
           data-bs-toggle="collapse" href="#filtresCollapse" role="button">
            <span><i class="bi bi-funnel me-2"></i>Filtres de recherche</span>
            <i class="bi bi-chevron-down"></i>
        </a>
    </div>
    <div class="collapse <?= array_filter($filters) ? 'show' : '' ?>" id="filtresCollapse">
        <div class="card-body">
            <form method="GET" action="/dons">
                <div class="row g-3">
                    <div class="col-md-4">
                        <label class="form-label small">Recherche</label>
                        <input type="text" name="search" class="form-control form-control-sm" 
                               placeholder="Article, donateur..." value="<?= htmlspecialchars($filters['search']) ?>">
                    </div>
                    <div class="col-md-2">
                        <label class="form-label small">Catégorie</label>
                        <select name="categorie_id" class="form-select form-select-sm">
                            <option value="">Toutes</option>
                            <?php foreach ($categories as $cat): ?>
                            <option value="<?= $cat['id'] ?>" <?= $filters['categorie_id'] == $cat['id'] ? 'selected' : '' ?>>
                                <?= htmlspecialchars($cat['nom']) ?>
                            </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-md-2">
                        <label class="form-label small">Disponibilité</label>
                        <select name="disponibilite" class="form-select form-select-sm">
                            <option value="">Tous</option>
                            <option value="disponible" <?= $filters['disponibilite'] == 'disponible' ? 'selected' : '' ?>>Disponible</option>
                            <option value="epuise" <?= $filters['disponibilite'] == 'epuise' ? 'selected' : '' ?>>Épuisé</option>
                        </select>
                    </div>
                    <div class="col-md-2">
                        <label class="form-label small">Date début</label>
                        <input type="date" name="date_debut" class="form-control form-control-sm" 
                               value="<?= htmlspecialchars($filters['date_debut']) ?>">
                    </div>
                    <div class="col-md-2">
                        <label class="form-label small">Date fin</label>
                        <input type="date" name="date_fin" class="form-control form-control-sm" 
                               value="<?= htmlspecialchars($filters['date_fin']) ?>">
                    </div>
                    <div class="col-md-12">
                        <button type="submit" class="btn btn-primary btn-sm">
                            <i class="bi bi-search me-1"></i>Rechercher
                        </button>
                        <a href="/dons" class="btn btn-outline-secondary btn-sm">
                            <i class="bi bi-x-circle me-1"></i>Réinitialiser
                        </a>
                    </div>
                </div>
            </form>
        </div>
    </div>
</div>

<div class="card">
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table table-hover mb-0">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Article</th>
                        <th>Catégorie</th>
                        <th>Qté Totale</th>
                        <th>Disponible</th>
                        <th>Donateur</th>
                        <th>Date Réception</th>
                        <th class="text-end">Actions</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if (empty($dons)): ?>
                    <tr>
                        <td colspan="8" class="text-center py-5 text-muted">
                            <i class="bi bi-gift fs-1 d-block mb-2"></i>
                            Aucun don trouvé
                        </td>
                    </tr>
                    <?php else: ?>
                        <?php foreach ($dons as $don): ?>
                        <tr>
                            <td><?= $don['id'] ?></td>
                            <td><strong><?= htmlspecialchars($don['article_nom']) ?></strong></td>
                            <td>
                                <span class="badge bg-light text-dark"><?= htmlspecialchars($don['categorie_nom']) ?></span>
                            </td>
                            <td><?= number_format($don['quantite_totale'], 2) ?> <?= $don['unite'] ?></td>
                            <td>
                                <span class="badge bg-<?= $don['quantite_disponible'] > 0 ? 'success' : 'secondary' ?>">
                                    <?= number_format($don['quantite_disponible'], 2) ?> <?= $don['unite'] ?>
                                </span>
                            </td>
                            <td><?= htmlspecialchars($don['donateur'] ?? 'Anonyme') ?></td>
                            <td><?= date('d/m/Y', strtotime($don['date_reception'])) ?></td>
                            <td class="text-end">
                                <a href="/dons/<?= $don['id'] ?>" class="btn btn-info btn-sm" title="Détails">
                                    <i class="bi bi-eye"></i>
                                </a>
                                <a href="/dons/edit/<?= $don['id'] ?>" class="btn btn-warning btn-sm" title="Modifier">
                                    <i class="bi bi-pencil"></i>
                                </a>
                                <a href="/dons/delete/<?= $don['id'] ?>" class="btn btn-danger btn-sm" 
                                   onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce don ?')" title="Supprimer">
                                    <i class="bi bi-trash"></i>
                                </a>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
        
        <div class="px-3">
            <?php 
            $baseUrl = '/dons';
            include __DIR__ . '/../components/pagination.php'; 
            ?>
        </div>
    </div>
</div>

<?php $content = ob_get_clean(); ?>
<?php include __DIR__ . '/../layout.php'; ?>
