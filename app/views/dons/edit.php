    ...
    <div class="col-lg-6">
        <div class="card">
            <div class="card-header">
                <i class="bi bi-pencil me-2"></i>Modifier le Don
            </div>
            <div class="card-body">
                <form action="/dons/update/<?= $don['id'] ?>" method="POST">
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="type_article_id" name="type_article_id" required>
                            ...
                            <?php endif; ?>
                                <option value="<?= $type['id'] ?>" <?= $type['id'] == $don['type_article_id'] ? 'selected' : '' ?>>
                    ...
                    <div class="mb-3">
                        <label for="quantite_totale" class="form-label">Quantité totale <span class="text-danger">*</span></label>
                        <input type="number" class="form-control" id="quantite_totale" name="quantite_totale" 
                               min="0.01" step="0.01" required value="<?= $don['quantite_totale'] ?>">
                        <small class="text-muted">Disponible actuellement: <?= number_format($don['quantite_disponible'], 2) ?> <?= $don['unite'] ?></small>
                    </div>
                    
                    <div class="mb-3">
                        <label for="donateur" class="form-label">Donateur</label>
                        <input type="text" class="form-control" id="donateur" name="donateur" 
                               value="<?= htmlspecialchars($don['donateur'] ?? '') ?>">
                    </div>
                    
                    <div class="mb-4">
                        <label for="description" class="form-label">Description</label>
                        <textarea class="form-control" id="description" name="description" rows="3"><?= htmlspecialchars($don['description'] ?? '') ?></textarea>
                    </div>
